...
Ein homogener Draht der Dichte $\rho$ mit kreisförmigem Querschnitt soll in einem homogenen Schwerefeld der Stärke $g$ aufgehängt werden.

Welche Form muss der Draht haben, wenn er überall bis zu seiner maximalen Zugspannung $\sigma$ gespannt sein soll und sein Radius im Aufhängepunkt $r_0$ betragen soll?
\begin{solution}
\[
...
Ein geneigter Spiegel bewege sich wie in der ersten Skizze dargestellt mit relativistischer Geschwindigkeit. Ein Lichtstrahl fällt unter einem Winkel $\alpha$ ein und wird unter einem Winkel $\beta$ reflektiert. Es lässt sich zeigen, dass die folgende Beziehung erfüllt ist:
\[
\sin{\alpha}-\sin{\beta}=\frac{v}{c}\cdot \sin{\theta}\cdot \sin{(\alpha + \beta)}.
\]
Nun bewege sich ein senkrechter Spiegel wie in der zweiten Skizze dargestellt.
\begin{abcenum}
\item Man zeige dass für den reflektierten Strahl
\[
\cos \beta = \frac{(1+(\frac{v}{c})^2)\cos \alpha -2\frac{v}{c}}{1-2\frac{v}{c}\cos \alpha + (\frac{v}{c})^2}
\]
gilt. \hinweis Man muss nicht die Lorentz-Transformation verwenden.
\item Man nehme an dass das einfallende Licht monochromatisch sei und die Frequenz $f$ habe. Wie groß ist die Frequenz des reflektierten Lichts und die relative Frequenzänderung $\frac{\Delta f}{f}$? Man gehe von $\alpha=30^\circ$ und $v=0.6 c$ aus.
\end{abcenum}
